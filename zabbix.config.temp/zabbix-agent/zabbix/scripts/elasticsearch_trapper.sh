#!/usr/bin/env bash
# grab , print and send mesos monitoring values to zabbix via zabbix_sender
# Author Email:  jyliu@dataman-inc.com
# Date: 2015-08-19


TMPFILE=`mktemp --suffix=-elasticsearch-monitoring`

ZABBIX_CONF=/etc/zabbix/zabbix_agentd.conf

PRE="mesos-"

trap "rm -f $TMPFILE ; exit" SIGHUP SIGINT SIGTERM  EXIT

curl 'http://127.0.0.1:9200/_cluster/health' 2>/dev/null|python -m json.tool 1>$TMPFILE

sed -i -e 's/["{}:,]//g' -e '/^[[:space:]]*$/d'  -e 's/^[[:space:]]*/'`hostname`' /g'  $TMPFILE

cat $TMPFILE
/usr/bin/zabbix_sender -c $ZABBIX_CONF -i $TMPFILE
